...
(function () {
  ...
  const TAB_PAGES = new Set(["brain", "buy", "sell", "opportunity", "industry"]);
  ...
  function updateHeader(page) {
    ...
    if (header) {
      const isBrain = page === "brain";
      header.classList.toggle("header-minimal", isBrain);
      header.classList.toggle("header-hidden", isBrain);
    }
  }
  ...
  function navigate(page) {
    ...
    // 底部 Tab 仅高亮五个主入口；问 Revo(search) 不抢占智脑高亮
    BottomNav.setActive(TAB_PAGES.has(target) ? target : null);
    updateHeader(target);
